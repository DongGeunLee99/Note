...
- [x] 알람/홈/휴지통 우측 패널 제거 — 빠른알람은 리스트 상단으로, 홈 알림테스트+팁 버튼은 최근기록 하단으로 이식
- [x] `PageHeader` 테두리 제거 + title 미표시, 우측 패널 없는 페이지는 버튼을 콘텐츠 컨테이너 안으로 이동(정렬 통일)
- [x] 메모 상세 모달 전면 개편 — 제목 상시편집(헤더에 배치), 위치 자동조회+직접입력, AI 버튼(탭+트리거 통합, 라벨 전환), 되돌리기/다시하기 쌍(`<`/`>`) — 상세는 [log.md](../log.md) 2026-08-14 (1)~(5) 참고
- [x] 캘린더 월/주/일 토글 버튼 활성 시 테두리 침범 버그 수정(z-index)
